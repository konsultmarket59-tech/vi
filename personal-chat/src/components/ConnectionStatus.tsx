...
const CODES: [RegExp, string][] = [
  ...
  [/ERR_TOO_MANY_RETRIES/, "Прокси отклоняет логин и пароль. Проверьте, что это логин от прокси, а не от Polza."],
  [
...
function fromStatus(status: number, what: string): string {
  ...
  if (status === 407) {
    return "Прокси требует логин и пароль, а те, что указаны, он не принял. Проверьте, что это логин от прокси, а не от Polza.";
  }
  if (status === 404) {
