...
server.listen(0, "127.0.0.1", () => {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      console.log("\nчестность про перенос на Тильду");
      // Самое важное в разделе: у API Тильды нет метода записи, и раздел не
      // должен намекать, что кнопка «залить на Тильду» где-то есть.
      const текст = await call(`document.body.textContent`);
      check("сказано, что сайт переносится руками",
        /переносится руками/.test(текст), текст.slice(0, 200));
      // Формы теперь вёрстаются — и раздел обязан сказать, что их надо
      // переподключить, иначе они молча потеряют заявки.
      check("сказано, что референсы агент видит", /он видит/.test(текст), "");
      check("названа причина — методы только на чтение",
        /все — на чтение|только на чтение/.test(текст), "");
      check("кнопки «залить на Тильду» нет",
        !/залить на Тильду|опубликовать на Тильд/i.test(текст), "");
      check("лимит 150 запросов назван", /150 запросов в час/.test(текст), "");
