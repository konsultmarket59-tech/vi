...
async function main() {
  ...
  console.log("\nограда markdown не должна попадать в блок");
  // Модель почти всегда оборачивает код в ```html … ```. Последствия
  // несоразмерны причине: CSS, начинающийся с ```css, недействителен ЦЕЛИКОМ —
  // блок остаётся вообще без стилей, а в просмотре видно текст ограды вместо
  // страницы. Со стороны это выглядит как «модель плохо рисует».
  const сОградой = sites.parseSite([
    "=== БЛОК 1 ===",
    "--- НАЗВАНИЕ ---",
    "Первый экран",
    "--- HTML ---",
  ...
  check("ограда снята с разметки",
    сОградой.blocks[0].html === "<h2>Дом у леса</h2>", JSON.stringify(сОградой.blocks[0].html));
  check("ограда снята со стилей",
    сОградой.blocks[0].css === "h2 { font-size: 32px }", JSON.stringify(сОградой.blocks[0].css));
  const собран = sites.buildBlockHtml(сОградой.blocks[0]);
  check("в собранном блоке ограды нет вовсе", !собран.includes("```"), собран);
  check("и стили теперь применяются", /\.dyn-1 h2\{/.test(собран), собран);
  check("одиночная ограда без пары тоже снимается",
    sites.stripFence("```html\n<p>a</p>") === "<p>a</p>", JSON.stringify(sites.stripFence("```html\n<p>a</p>")));
  check("текст без ограды не портится",
    sites.stripFence("<p>обычный</p>") === "<p>обычный</p>");

  console.log("\nлоготип попадает на сайт");
  // Логотип агент видит, но вставить не может: путь с компьютера на сайте
  // превращается в пустое место. Поэтому метка, а файл подставляет приложение.
  const сЛоготипом = [{ id: "1", html: '<img src="[ЛОГОТИП: белый на тёмном]"><img src="[ЛОГОТИП]">', css: "" }];
  const меткиЛого = sites.collectLogoMarks(сЛоготипом);
  check("метки логотипа собраны с уточнениями",
    меткиЛого.length === 2 && меткиЛого[0].hint === "белый на тёмном", JSON.stringify(меткиЛого));
  const файлыЛого = [
    { name: "logo-чёрный.svg", kind: "image", path: "/x/1.svg" },
    { name: "logo-белый-на-тёмном.png", kind: "image", path: "/x/2.png" },
  ];
  check("вариант выбран по уточнению",
    sites.pickLogo(файлыЛого, "белый на тёмном").name === "logo-белый-на-тёмном.png",
    sites.pickLogo(файлыЛого, "белый на тёмном").name);
  check("без уточнения берётся первый",
    sites.pickLogo(файлыЛого, "").name === "logo-чёрный.svg");
  check("когда логотипов нет — ничего не выдумывается", sites.pickLogo([], "белый") === null);
  const сПодставленным = sites.applyLogos(сЛоготипом, { "белый на тёмном": "data:image/png;base64,AAA" });
  check("логотип встал строкой данных",
    сПодставленным.blocks[0].html.includes("data:image/png;base64,AAA"), сПодставленным.blocks[0].html);
  check("ненайденный вариант назван, а не оставлен молча",
    сПодставленным.missing.includes("без уточнения"), JSON.stringify(сПодставленным.missing));
  check("доктрина требует ставить логотип",
    /ИСПОЛЬЗУЙ ЕГО/.test(sites.SITE_AGENT_PROMPT));
