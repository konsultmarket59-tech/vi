...
| `LLM_API_KEY` | Ключ Polza.ai — используется, когда пул `hooks.txt` исчерпан |
...
- **когда пул полностью исчерпан, добирает недостающие хуки через LLM**
  (Polza.ai, консалтинговый тон в духе McKinsey/BCG — см. `LLM_HOOK_PROMPT`
  в `generate_reels.py`). Эти хуки тоже попадают в историю, чтобы не
  повторяться между днями.
